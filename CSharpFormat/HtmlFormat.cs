...
namespace Manoli.Utils.CSharpFormat
{
	...
	public class HtmlFormat : SourceFormat
	{
		...
		public HtmlFormat()
		{
            ...
            //MSage: 20150423 
            const string regAngularInlineBinding = @"{{.*?}}";
			...
			string regAll = "(" + regJavaScript + ")|(" + regComment + ")|(" 
				+ regAngularInlineBinding + ")|(" + regAspCode + ")|(" 
		...
		{
            //TODO: MSage: Changed from kwrd to attrvalue 20150423
			if(match.Groups[1].Success) //attribute value
				return "<span class=\"attrvalue\">" + match.ToString() + "</span>";
		...
		protected override string MatchEval(Match match)
		{
            ...
            if (match.Groups[3].Success) //asp tag
            {
                // Bit of a fudge to exclude braces from color coding by hardcoding
                // outside of span and triming match string
                return "{{<span class=\"angular\">" + match.ToString().Trim('{', '}') + "</span>}}";
            }
